...
torch.cuda.manual_seed_all(
    SEED
)  # seed all GPUs — no-op on CPU; needed for GPU reproducibility
...
def load_split(
    ...
    if "X_feat" not in data:
        raise KeyError(
            f"'X_feat' not found in {path}. Run scripts/preprocess.py first."
        )
    saved_ver = data.get("feature_scaler_version")
    if (
        expected_version is not None
        and saved_ver is not None
        and saved_ver != expected_version
    ):
        raise ValueError(
...
def _setup_device_amp(logger: logging.Logger):
    ...
    if _AMP_DEVICE:
        ...
        def _amp_ctx():
            return autocast(_AMP_DEVICE, enabled=use_amp)
    else:
...
def train(
    ...
    if X_train.shape[-1] != INPUT_FEATURES:
        raise ValueError(
            f"Data has {X_train.shape[-1]} features, config expects {INPUT_FEATURES}."
        )
    if X_feat_train.shape[-1] != SPECTRAL_FEAT_DIM:
        raise ValueError(
            f"Feature dim {X_feat_train.shape[-1]}, config expects {SPECTRAL_FEAT_DIM}."
        )
    ...
    logger.info(
        f"Config: lr={LR}, batch={BATCH_SIZE}, epochs={epochs}, patience={PATIENCE}"
    )
...
def train_long(
    ...
    patch_size: int = LONG_PATCH_SIZE,
    patch_stride: int = LONG_PATCH_STRIDE,
    weighted_loss: bool = False,
    eol_weight_scale: float = 2.0,
    cosine_t0: int = 25,
    ...
    warmup_cosine_t0: int = 0,
    d_state: int = LONG_D_STATE,
) -> None:
    ...
    X_train, X_feat_train, y_train = load_split(
        os.path.join(data_dir, "train.pt"), FEATURE_SCALER_VERSION_LONG
    )
    X_val, X_feat_val, y_val = load_split(
        os.path.join(data_dir, "val.pt"), FEATURE_SCALER_VERSION_LONG
    )
    X_test, X_feat_test, y_test = load_split(
        os.path.join(data_dir, "test.pt"), FEATURE_SCALER_VERSION_LONG
    )
    seq_len = X_train.shape[1]
    logger.info(
        f"  Train {len(X_train)} | Val {len(X_val)} | Test {len(X_test)} | seq_len={seq_len}"
    )
    ...
    if benchmark and device.type == "cuda":
        ...
        logger.info(
            "cuDNN benchmark: ON (non-deterministic — reproducibility disabled)"
        )
    ...
    torch.manual_seed(SEED)
    num_tokens = (
        (seq_len - patch_size) // patch_stride + 1 if patch_size > 1 else seq_len
    )
    logger.info(
        ...
        if patch_size > 1
        else f"Patch: disabled (patch_size=1, {seq_len} tokens)"
    )
    ...
    if compile_model:
        try:
            ...
            logger.info(
                "torch.compile: ON (default, dynamic=True) — handles variable L across warmup stages without recompilation"
            )
    ...
    logger.info(
        f"Warmup scheduler: CosineAnnealingWarmRestarts(T_0={warmup_t0}, T_mult=1, eta_min=1e-6)"
    )
    ...
    if weighted_loss:
        ...
        def criterion(pred, target):  # type: ignore[assignment]
            return _soh_weighted_smooth_l1(
                pred, target, beta=0.02, weight_scale=eol_weight_scale
            )

        logger.info(
            f"Loss: SmoothL1(beta=0.02) + EOL upweight (scale={eol_weight_scale})"
        )
    else:
    ...
    GRAD_CLIP = 1.0
    logger.info(
        f"Optim: AdamW(lr={LR}, weight_decay={weight_decay}) | dropout={dropout} | jitter={jitter}"
    )
    ...
    patience_counter = 0
    swa_state = (
        None  # running weight average over the final stage's tail (SWA, Izmailov 2018)
    )
    swa_n = 0
    swa_start = 0
    for si, stage_len in enumerate(stages):
        ...
        logger.info(
            f"[stage {si + 1}/{len(stages)}] L={stage_len} epochs={n_epochs}{'  (final)' if is_final else ''}"
        )
        for epoch in range(1, n_epochs + 1):
            ...
            val_metrics = evaluate(
                model, Xval_s, X_feat_val, y_val, device, batch_size=eval_batch
            )
            val_loss = val_metrics["loss"]
            ...
            if is_final:
                ...
                if swa and epoch >= swa_start:
                    ...
                    if swa_state is None:
                        swa_state = {
                            k: v.detach().float().clone() for k, v in cur.items()
                        }
                        swa_n = 1
                    else:
                ...
                if patience_counter >= final_patience:
                    logger.info(
                        f"Early stopping at epoch {epoch} (patience={final_patience})"
                    )
                    break
    ...
    if swa and swa_state is not None and swa_n > 0:
        ...
        swa_val = evaluate(
            model, X_val, X_feat_val, y_val, device, batch_size=eval_batch
        )
        logger.info(
    ...
    test_metrics = evaluate(
        model, X_test, X_feat_test, y_test, device, batch_size=eval_batch
    )
    logger.info("-" * 55)
    logger.info(f"Test MAE : {test_metrics['mae']:.4f}%  (target < 2.0%)")
    logger.info(f"Test RMSE: {test_metrics['rmse']:.4f}%  (target < 3.0%)")
    ...
    torch.save(
        {
            ...
            "warmup_cosine_t0": warmup_cosine_t0
            if warmup_cosine_t0 > 0
            else stage_epochs,
            "optimizer": "adamw",
...
def load_rul_split(path: str) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    if "y" not in data or "X" not in data:
        raise KeyError(
            f"'X'/'y' not found in {path}. Run scripts/preprocess_rul.py first."
        )
...
def train_rul(data_dir: str, epochs: int, log_dir: str, pooling: str = "last") -> None:
    ...
    logger.info(
        f"  Train {len(X_train)} | Val {len(X_val)} | Test {len(X_test)} | lookback={X_train.shape[1]} cycles"
    )
    logger.info(
        f"  EOL=SOH<={EOL_SOH}% | RUL range train {y_train.min():.0f}..{y_train.max():.0f} cycles | scale={RUL_SCALE}"
    )

    if X_train.shape[-1] != RUL_FEAT_DIM:
        raise ValueError(
            f"Feature dim {X_train.shape[-1]}, config expects {RUL_FEAT_DIM}."
        )
    ...
    torch.manual_seed(SEED)
    model = RULPredictor(
        feat_dim=RUL_FEAT_DIM, d_model=D_MODEL, d_state=D_STATE, pooling=pooling
    ).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=LR, weight_decay=1e-5)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="min", factor=0.5, patience=5, min_lr=1e-6
    ...
    logger.info(
        f"{'Epoch':>6}  {'TrainLoss':>10}  {'ValLoss':>10}  {'ValMAE(cyc)':>11}  {'ValRMSE(cyc)':>12}"
    )
    logger.info("-" * 60)
    for epoch in range(1, epochs + 1):
        model.train()
        ...
        val_loss = val_metrics["loss"]
        logger.debug(
            f"{epoch:>6}  {train_loss:>10.6f}  {val_loss:>10.6f}  {val_metrics['mae']:>11.4f}  {val_metrics['rmse']:>12.4f}"
        )
        if epoch % 10 == 0:
            logger.info(
                f"{epoch:>6}  {train_loss:>10.6f}  {val_loss:>10.6f}  {val_metrics['mae']:>11.4f}  {val_metrics['rmse']:>12.4f}"
...
def _lobo(
    ...
    logger.info(
        f"Pool: {len(ids)} batteries | held-out folds: {fold_ids} | lookback={blob.get('lookback')} | scale={scale} | unit={unit}"
    )
    ...
    for held in fold_ids:
        ...
        if len(Xte) == 0 or len(Xtr) == 0:
            logger.warning(
                f"[hold {held}] empty fold (train {len(Xtr)} test {len(Xte)}) — skipped"
            )
            continue
        ...
        Xt = torch.tensor(Xtr_s[tr_idx])
        yt = torch.tensor(ytr[tr_idx])
        Xv = torch.tensor(Xtr_s[val_idx])
        yv = torch.tensor(ytr[val_idx])
        Xe = torch.tensor(Xte_s)
        ye = torch.tensor(yte)
        ...
        torch.manual_seed(SEED)
        model = RULPredictor(
            feat_dim=nf, d_model=D_MODEL, d_state=D_STATE, pooling=pooling
        ).to(device)
    ...
    if results:
        ...
        logger.info("-" * 60)
        logger.info(
            f"LOBO macro-avg over {len(results)} folds: "
...
def train_rul_lobo(
    data_dir: str, epochs: int, log_dir: str, pooling: str = "last"
) -> None:
    _lobo(
        os.path.join(data_dir, "by_battery.pt"),
        epochs,
        log_dir,
        scale=RUL_SCALE,
        unit="cycles",
...
def train_forecast_lobo(
    ...
    _lobo(
        os.path.join(data_dir, "by_battery_forecast.pt"),
        epochs,
        log_dir,
        scale=100.0,
        unit="%SOH",
...
def train_forecast_delta(
    ...
    blob = torch.load(
        os.path.join(data_dir, "by_battery_forecast.pt"), weights_only=False
    )
    batteries = blob["batteries"]
    if "anchor" not in next(iter(batteries.values())):
        raise KeyError(
            "by_battery_forecast.pt has no 'anchor' — rerun scripts/preprocess_forecast.py."
        )
    ids = list(batteries.keys())
    fold_ids = ids if holdouts is None else [h for h in holdouts if h in batteries]
    logger.info(
        f"Pool: {len(ids)} batteries | folds: {fold_ids} | lookback={blob.get('lookback')} | horizon=+{blob.get('horizon')} | delta-scale={scale}"
    )
    ...
    for held in fold_ids:
        ...
        vi, ti = idx[:nv], idx[nv:]
        Xt = torch.tensor(Xtr_s[ti])
        dt = torch.tensor(dtr[ti])
        Xv = torch.tensor(Xtr_s[vi])
        dv = torch.tensor(dtr[vi])
    ...
    if results:
        logger.info("-" * 60)
        logger.info(
            f"DELTA macro-avg: model MAE {np.mean([r[1] for r in results]):.3f} | "
...
def _parse_warmup_stages(value: str) -> list[int]:
    """GH-43: '2048,4096' -> [2048, 4096]; argparse-friendly error on junk."""
    try:
        stages = [int(s) for s in value.split(",") if s.strip()]
    except ValueError as exc:
        raise argparse.ArgumentTypeError(
            f"--warmup-stages expects comma-separated ints (e.g. '2048,4096'), got {value!r}"
        ) from exc
    if not stages:
        raise argparse.ArgumentTypeError(
            "--warmup-stages must contain at least one stage"
        )
    if any(s <= 0 for s in stages):
        raise argparse.ArgumentTypeError("--warmup-stages values must be positive")
    return stages
...
def main() -> None:
    ...
    parser.add_argument(
        "--rul", action="store_true", help="Train cycle-level RUL predictor (GH-13)"
    )
    parser.add_argument(
        "--lobo",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=None,
        help="GH-43: comma-separated warmup stage lengths for --long "
        f"(e.g. '2048,4096'; default = {WARMUP_STAGES}). Stages > seq_len "
        "are dropped and seq_len is always appended as the final stage.",
    )
    ...
    if args.forecast:
        ...
        train_rul_lobo(
            args.data_dir or "data/processed_rul",
            args.epochs,
            args.log_dir,
            pooling=args.pooling,
        )
