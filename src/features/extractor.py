...
def _spectral_gini(power: np.ndarray) -> float:
    ...
    return float(
        np.clip((2.0 * (ranks * x).sum()) / (n * total) - (n + 1) / n, 0.0, 1.0)
    )
...
def compute_ic_curve_and_discharge_progress(
    voltage: np.ndarray, current: np.ndarray, time: np.ndarray
) -> tuple[np.ndarray, np.ndarray]:
    """IC curve (dQ/dV) + discharge progress for ONE discharge cycle.

    These are the 2 derived columns trained into scaler_long.pkl (v2.1+,
    features=["voltage","current","temperature","time","ic_curve","discharge_progress"]).
    Both MUST be computed within a single cycle's scope (dt/dQ integration
    resets at cycle start) — calling this across a window that spans multiple
    concatenated cycles produces an out-of-distribution signal at each cycle
    boundary, since the underlying integration would run through the
    voltage/current discontinuity where one cycle ends and the next begins.

    Args:
        voltage: (T,) raw unscaled voltage (V), one discharge cycle.
        current: (T,) raw unscaled current (A, negative = discharge), same cycle.
        time:    (T,) raw cumulative time within THIS cycle (s).

    Returns:
        (ic_curve, discharge_progress), each (T,) float32.
        ic_curve: |I|*dt / |dV|, clipped [0, 20] A·s/V.
        discharge_progress: cumulative discharged charge / total charge, in [0, 1].
    """
    ...
    return ic, discharge_progress


# ---------------------------------------------------------------------------
# Window-local SOC (GH-54)
# ---------------------------------------------------------------------------


def compute_soc_percent(
    current: np.ndarray, time: np.ndarray, nominal_capacity_ah: float = 2.0
) -> np.ndarray:
    """Window-local State of Charge % via Coulomb counting.

    SOC is defined RELATIVE TO THE WINDOW: 100% at the first row, then
    decreases by the charge drawn since that row. This is intentionally NOT
    the physical full-cycle SOC — run_inference() is stateless (one isolated
    window per request, no per-battery session), so the only definition that
    is exactly reproducible at inference time is one computed from the window
    itself (same pattern as compute_ic_feature/compute_phase_mask above).

    SOC(t) = 100 - (integral of |I| dt from window start to t) / C_nominal * 100

    Args:
        current:             (L,) raw unscaled current signal (A).
        time:                (L,) raw cumulative time within the cycle (s).
        nominal_capacity_ah: nominal capacity (Ah) — NASA cells: 2.0.

    Returns:
        np.ndarray of shape (L,), float32, clipped to [0, 100]. First element
        is always 100.0.
    """
    from scipy.integrate import cumulative_trapezoid

    t_hours = time.astype(np.float64) / 3600.0
    ah_drawn = cumulative_trapezoid(
        np.abs(current.astype(np.float64)), t_hours, initial=0.0
    )
    soc = 100.0 - ah_drawn / nominal_capacity_ah * 100.0
    return np.clip(soc, 0.0, 100.0).astype(np.float32)
