...
_CATEGORY_LABELS = {
    _CAT_CHARGING: "charging fault",
    _CAT_OVERHEAT: "overheating",
...
def _score_skill(candidate: StaffCandidate, category: int) -> tuple[int, str]:
    ...
    if matched_primary:
        return _W_SKILL_PRIMARY, f"matches the primary skill '{sorted(matched_primary)[0]}'"

    matched_secondary = owned & (secondary - {"general"})
    if matched_secondary:
        return _W_SKILL_SECONDARY, f"holds the related skill '{sorted(matched_secondary)[0]}'"

    if "general" in owned:
        return _W_SKILL_GENERAL_ONLY, "generalist skill set"

    return 0, "no skill matching this fault type"
...
def _score_tier(candidate: StaffCandidate, priority: int) -> tuple[int, str]:
    ...
    if gap == 0:
        return _W_TIER_EXACT, f"{tier_label} matches the {prio_label} requirement"
    if gap == 1:
        return _W_TIER_ONE_ABOVE, f"{tier_label} is above the {prio_label} requirement"
    return 0, f"{tier_label} far exceeds the {prio_label} requirement"
...
def _score_load(candidate: StaffCandidate) -> tuple[float, str]:
    ...
    if candidate.max_concurrent <= 0:
        return float(_W_LOAD_MAX), "no ticket limit configured"
...
def run_suggest_staff(req: SuggestStaffRequest) -> SuggestStaffResponse:
    ...
    if not req.candidates:
        return SuggestStaffResponse(
            suggestions=[], note="No technician is available to take the ticket."
        )
    ...
    if not eligible:
        ...
        if blocked_by_tier and not blocked_by_load:
            note = f"No technician meets the tier required for a {prio_label} ticket."
        elif blocked_by_load and not blocked_by_tier:
            note = "Every suitable technician is at full workload."
        else:
            note = (
                f"No suitable candidate: {blocked_by_tier} below the required tier, "
                f"{blocked_by_load} at full workload."
            )
    ...
    if best_skill_pts <= _W_SKILL_GENERAL_ONLY:
        note = f"Nobody holds a dedicated {cat_label} skill — consider assigning a supporter as well."
