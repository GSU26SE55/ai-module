...
> Khôi phục 2026-07-09 (bản cũ bị xóa khỏi repo, nội dung không đổi + thêm
> ghi chú split GH-88 ở §3.1).
...
> Ghi chú nội bộ (không đưa vào bài): đây là split CŨ 23/2/1 — protocol chính
> thức của bài báo. Production sau GH-88 (08/07) dùng 24/1/1 (B0047 → train);
> trong bài, v1.6 chỉ xuất hiện ở phần data-centric §4.2/§4.4b với chú thích split.
...
| Thông số | Nguồn |
|----------|-------|
| 6 kênh long = 4 base + IC + discharge progress | `scripts/preprocess_long.py` dòng 72–76, 235 (plan cũ ghi "phase mask" là SAI) |
...
| lr 5e-4, batch 32 | `scripts/train.py` (BATCH_SIZE/LR) |
...
| Window-30: 6 features, 79,467 params, d_state 16, epochs 50/patience 10 | `src/core/config.py` FEATURES + đếm params + rules |
| Số token 511 = (4096−16)/8+1 | công thức từ patch_size/stride checkpoint (⬜ verify padding khi rảnh) |
| Nhãn anomaly rate-based p90 = 0.4833 %SOH/cycle | `logs/nckh/anomaly/table5.md` (GH-70) |
| Ambient temps 4/22/24/43(/44) | `metadata.csv` groupby (verify 07/07) |
| MC Dropout 20 lượt | pipeline inference (GH-62 batch 20 forward) |
